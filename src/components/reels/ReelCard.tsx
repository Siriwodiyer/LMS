...
export const ReelCard: React.FC<ReelCardProps> = ({
  ...
  const handleTimeUpdate = () => {
    if (videoRef.current) {
      ...
      // Trigger watched completion automatically after 4 seconds
      if (current >= 4 && isActive && !isCompleted) {
  ...
  return (
    <div className="relative w-full max-w-[380px] h-[675px] max-h-[calc(100vh-200px)] aspect-[9/16] rounded-3xl overflow-hidden bg-slate-950 border border-slate-800 shadow-2xl flex flex-col select-none mx-auto">
      ...
      >
        ...
        {/* Floating Gradient Backdrop for text readability */}
        ...
        <div className="absolute top-4 left-4 right-4 flex items-center justify-between z-20">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-blue-600/90 text-white shadow-xs backdrop-blur-md border border-blue-400/30">
              Reel {reelIndex + 1} of {totalReels}
            </span>
            <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-black/60 text-slate-200 border border-white/10 backdrop-blur-md">
              {reel.category}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm cursor-pointer ${
                isCompleted
            ...
            <button
              ...
              className="w-8 h-8 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-white border border-white/15 hover:bg-black/80 transition-all cursor-pointer"
        ...
        <div className="absolute left-4 right-16 bottom-4 z-20 text-left">
          ...
          <div className="flex items-center gap-2.5 mb-2">
            <div className="w-8 h-8 rounded-full bg-blue-600 text-white font-bold text-xs flex items-center justify-center border border-white/30 shrink-0 shadow-md">
          ...
          {/* Title */}
          <h2 className="font-bold text-sm text-white leading-snug drop-shadow line-clamp-2 mb-1.5">
            {reel.title}
          </h2>
          ...
          <div className="mt-2.5 flex items-center gap-1.5 text-[11px] text-blue-200">
